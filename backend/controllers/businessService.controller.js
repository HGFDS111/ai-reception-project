...
export const addServiceToBusiness = async (req, res) => {
  try {
    ...
    const serviceTemplate = await ServiceTemplate.findByPk(serviceTemplateId);
    ...
    console.error(error);
    res.status(500).json({ message: 'Error adding service to business' });
  }
};
...
export const getBusinessServices = async (req, res) => {
  try {
    ...
    console.error(error);
    res.status(500).json({ message: 'Error getting business services' });
  }
};